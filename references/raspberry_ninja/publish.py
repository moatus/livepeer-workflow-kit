...
def select_audio_sink(default_sink: str = "autoaudiosink") -> str:
    """Choose the most appropriate local audio sink."""
    forced_sink = os.environ.get("RN_FORCE_AUDIO_SINK")
    if forced_sink:
        printc(f"\n ! Forcing audio sink via RN_FORCE_AUDIO_SINK={forced_sink}", "0AF")
        return forced_sink
    return default_sink
...
class WebRTCClient:
    ...
    def __init__(self, params):
        ...
        try:
            self.fdsink_fd = int(os.environ.get("RN_FDSINK_FD", "1"))
        except ValueError:
            self.fdsink_fd = 1
            ...
            if self.ndiout:
                ...
                if self.ndiout:
                   ...
                   pass
                    ...
                    sink.set_property("fd", self.fdsink_fd)
                ...
                if self.ndiout:
                    ...
                    audio_sink = select_audio_sink("autoaudiosink")
                    if self.recording_enabled:
                        filename = f"{self.record}_{str(int(time.time()))}_audio.webm"
                        out = Gst.parse_bin_from_description(
                            "queue ! rtpopusdepay ! opusparse ! tee name=audio_record_tee "
                            "audio_record_tee. ! queue ! opusdec ! audioconvert ! "
                            f"audioresample ! audio/x-raw,format=S16LE,rate=48000 ! {audio_sink} "
                            "audio_record_tee. ! queue ! webmmux ! "
                            f"filesink name=filesinkaudio location={filename}",
                            True,
                        )
                        self.recording_files.append(filename)
                        printc(f"   📁 Audio output: {filename}", "77F")
                    else:
                        out = Gst.parse_bin_from_description(
                            "queue ! rtpopusdepay ! opusparse ! opusdec ! "
                            f"audioconvert ! audioresample ! audio/x-raw,format=S16LE,rate=48000 ! {audio_sink}",
                            True,
                        )
                    ...
                    #if "OPUS" in name:
                    out = Gst.parse_bin_from_description(f"queue ! rtpopusdepay ! opusparse ! opusdec ! audioconvert ! audioresample ! audio/x-raw,format=S16LE,rate=48000,channels=1 ! fdsink fd={self.fdsink_fd}", True)
